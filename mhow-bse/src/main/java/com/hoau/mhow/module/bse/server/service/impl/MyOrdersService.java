...
import com.hoau.mhow.invokews.server.ws.omsorder.EiNetOrderReq;
...
import com.hoau.wechat.util.Verhoeff;
...
public class MyOrdersService implements IMyOrdersService {
	...
	public void updateOrderStatus(Integer orderId) {
		NetOrderEntity netOrder = null;
		EiNetOrderReqModel netReqOrder = new EiNetOrderReqModel();
		try {
			netOrder = netOrderDao.queryOrderDetail(orderId);
			//订单状态设为取消
...
//			}
			iBseOperationLogService.insert(netOrder, 2);
		} catch (NumberFormatException e) {
			logger.info("撤销订单客户id:"+netOrder.getEinoEbccId());
			logger.info("撤销订单号:"+netOrder.getEinoContractNo());
			logger.info(JsonUtils.toJson(netReqOrder));
			e.printStackTrace();
		}
	}
	...
	public void deleteOrder(Integer orderId) {
		try {
			netOrderDao.deleteOrder(orderId);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public List<MyOrdersVo> getPrintOrders(String orderIdList) {
		...
		try {
			...
			for(String orderId : orderIdArr){
				NetOrderEntity entity = netOrderDao.queryOrderDetail(Integer.parseInt(orderId));
...
//		maps.put("einoConsigneeBranchesName", "收货网点");
	...
	public NetOrderEntity queryOrderDetail(Integer orderId) {
		NetOrderEntity entity = netOrderDao.queryOrderDetail(orderId);
		//处理收发货人省市区
		...
		if(StringUtil.isNotEmpty(entity.getEinoShipperEbsaTel())){
			String [] shipperTel = entity.getEinoShipperEbsaTel().split("-");
			if(shipperTel.length!=0&&shipperTel.length==2){
				entity.setEinoShipperEbsaTel(shipperTel[1]);
			}
		}
		entity.setEinoShipperEbsaAreaCode(entity.getEinoShipperEbsaAreaCode()==null?"":entity.getEinoShipperEbsaAreaCode());
		...
		if(StringUtil.isNotEmpty(entity.getEinoConsigneeEbsaTel())){
			String [] consigneeTel = entity.getEinoConsigneeEbsaTel().split("-");
			if(consigneeTel.length!=0&&consigneeTel.length==2){
				entity.setEinoConsigneeEbsaTel(consigneeTel[1]);
			}
		}
...
//	}
	...
	public List<MyOrdersVo> selectOrdersList(QueryOrderVo queryOrderVo) {
		QueryOrderVo queryVo = new QueryOrderVo();
		if(StringUtils.isEmpty(queryOrderVo.getCustomerId())){
			throw new OBHException("用户id为空");
		}
		if(!StringUtil.isEmpty(queryOrderVo.getDoneType())){
			Integer pageNo = queryOrderVo.getOffset(),pageSize = 10;
			queryVo.setDoneType(queryOrderVo.getDoneType());
			queryVo.setPageSize(pageSize);
			queryVo.setOffset((pageNo-1) * pageSize);
		}
		
		if(!StringUtil.isEmpty(queryOrderVo.getStartQueryTime())){
			queryVo.setStartQueryTime(getQueryDateParam(Constant.QUERY_ORDER_DAYS.get(queryOrderVo.getStartQueryTime())));
			queryVo.setEndQueryTime(DateUtils.getCurDateTime());
		}
		
		if(!StringUtil.isEmpty(queryOrderVo.getOrderNo())){
			queryVo.setOrderNo(queryOrderVo.getOrderNo());
		}
		
		queryVo.setCustomerId(queryOrderVo.getCustomerId());
		List<NetOrderEntity> list = netOrderDao.queryOrderList(queryVo);
		return toMyOrderVo(list);
	}
	@Override
	public NetOrderEntity createOrderByContact(ContactsEntity contactsEntity) {
		NetOrderEntity order = new NetOrderEntity();
		// TODO crateOrder
		 if(contactsEntity.getEbsaType().equals(ContactsType.SHIPPER)){
		    	order.setEinoShipperEbsaContact(contactsEntity.getEbsaContact());
		    	order.setEinoShipperEbsaMobile(contactsEntity.getEbsaMobile());
//		    	String shipperTel = "";
//				if(StringUtil.isNotEmpty(contactsEntity.getEbsaContactAreaCode()) &&  StringUtil.isNotEmpty(contactsEntity.getEbsaContactTel())){
//					shipperTel = contactsEntity.getEbsaContactAreaCode()+"-"+contactsEntity.getEbsaContactTel();
//				}
		    	order.setEinoShipperEbsaAreaCode(contactsEntity.getEbsaContactAreaCode());
				order.setEinoShipperEbsaTel(contactsEntity.getEbsaContactTel());
		    	order.setEinoShipperEbspNameCn(contactsEntity.getEbsaCompany());
		    	order.setEinoShipperEbsaAddress(contactsEntity.getEbsaAddress());
		    	order.setShipperDistrict(contactsEntity.getEbsaEbpvName()+"-"+contactsEntity.getEbsaEbplName()+"-"+contactsEntity.getEbsaEbcoName());
		    	order.setEinoEscoSecondCode(contactsEntity.getEbsaEscoSecondCode());
		    	order.setEinoEscoSecondName(contactsEntity.getEbsaEscoSecondName());
		    	order.setEinoSecondDistrict(contactsEntity.getEbsaDeptDistrict());
		    	String secondDistrict = order.getEinoSecondDistrict();
				if(secondDistrict != null && secondDistrict != ""){
					Map<String,String> map = new HashMap<String,String>();
					if(!StringUtil.isEmpty(secondDistrict)){
						String[] params = secondDistrict.split("-");
						if(params.length>1){
							...
							List<DepartmentVo> departmentVos = companyMatchService.queryDeptByDistrictName(map);
							for(DepartmentVo vo : departmentVos){
								if(vo.getLogistCode().equals(order.getEinoEscoSecondCode())){
									order.setCompanyMsg(vo.getAddressDetail() + "|" + vo.getPhone());
									break;
								}
							}
						}
		    	...
		    	order.setEinoConsigneeEbsaContact(contactsEntity.getEbsaContact());
		    	order.setEinoConsigneeEbsaMobile(contactsEntity.getEbsaMobile());
		    	order.setEinoConsigneeEbsaAreaCode(contactsEntity.getEbsaContactAreaCode());
		    	order.setEinoConsigneeEbsaTel(contactsEntity.getEbsaContactTel());
		    	order.setEinoConsigneeEbspNameCn(contactsEntity.getEbsaCompany());
		    	order.setEinoConsigneeEbsaAddress(contactsEntity.getEbsaAddress());
		    	order.setConsigneeDistrict(contactsEntity.getEbsaEbpvName()+"-"+contactsEntity.getEbsaEbplName()+"-"+contactsEntity.getEbsaEbcoName());
		    }
		return order;
	}
	/**
	 ...
	 * @author Dy
	 */
	public String getQueryDateParam(Integer days){
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(new Date());
		calendar.add(Calendar.DAY_OF_MONTH, -(days));
		return DateUtils.formatDateYYYYMMDDHHmmss(calendar.getTime());
	}
	/***********调试结束*************/
