...
import com.hoau.wechat.md5.MD5BASE64;
...
public class ForgotAction extends AbstractAction {
	...
	public String phoneRetrieve() {
		try {
			...
			String phoneVerCode = (String) ctx.getSession().get(
					SessionConstants.SESSION_VERCODE_KEYS.PHONE_VERCODE);
			if (phone == null || phone.equals("")) {
				errorType = "phone";
				setThisErrorMsg("手机号码为空!");
				return "checkError";
			} else {
				if(phoneVerCode != null && registVerCode != null && phoneVerCode.equalsIgnoreCase(registVerCode)){
					CustomerContactEntity vo = new CustomerContactEntity();
					vo.setEbccMobile(this.phone);
			...
			ctx.getSession().put(
					SessionConstants.SESSION_VERCODE_KEYS.PHONE_VERCODE, null);
		} catch (Exception ex) {
			logger.error("ForgotAction phoneRetrieve error : "
					+ ex.getMessage());
			ex.printStackTrace();
			return "checkError";
		}
	...
	public String sendNewPhoneVerCodeJson() {
		try {
			if(!phoneRetrieve().equals("phoneRetrieve")){
			...
			Object obj = ctx
					.getSession()
					.get(SessionConstants.SESSION_VERCODE_KEYS.PHONE_VERCODE_SEND_TIME);
			...
			if (obj == null) {
				...
				// 将第一次获取验证码时间保存
				ctx.getSession()
						.put(SessionConstants.SESSION_VERCODE_KEYS.PHONE_VERCODE_SEND_TIME,
								DateUtils.getCurTimeMillis());
				sendMsg = true;
			} else {
				// 第一次获取验证码时间
			...
			if (sendMsg && phone != null && !phone.equals("")) {
				...
				SMS sms = new SMS(phone, "您本次找回密码验证码为：" + phoneSmsVercode
						+ "，请您在30分钟内完成验证，如有疑问请致电：400-808-6666");
				forgotService.sendSms(sms);
				// 将手机MD5验证码保存至session
				ctx.getSession()
						.put(SessionConstants.SESSION_VERCODE_KEYS.PHONE_SMS_MD5_VERCODE,
								MD5BASE64.base64MD5(phone + phoneSmsVercode,
										"UTF-8"));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			logger.error("ForgotAction sendPhoneVerCodeJson error : "
					+ ex.getMessage());
		}
		return "sendPhoneVerCodeJson";
	}
	...
	public String checkCountDownPhoneJson() {
		try {
			...
			ex.printStackTrace();
			logger.error("ForgotAction checkCountDownPhoneJson error : "
					+ ex.getMessage());
		}
	...
	public String emailRetrieve() {
		try {
			...
			String emailVerCode = (String) ctx.getSession().get(
					SessionConstants.SESSION_VERCODE_KEYS.EMAIL_VERCODE);
			if (email == null || email.equals("")) {
				errorType = "email";
				setThisErrorMsg("邮箱地址为空!");
				return "checkError";
			} else {
				if(emailVerCode != null && registVerCode != null && emailVerCode.equalsIgnoreCase(registVerCode)){
					CustomerContactEntity vo = new CustomerContactEntity();
			...
			ctx.getSession().put(
					SessionConstants.SESSION_VERCODE_KEYS.EMAIL_VERCODE, null);
		} catch (Exception ex) {
			logger.error("ForgotAction emailRetrieve error : "
					+ ex.getMessage());
			ex.printStackTrace();
			return "checkError";
		}
	...
	public String sendEmailVerCodeJson() {
		try {
			if(!emailRetrieve().equals("emailRetrieve")){
				return "checkError";
			}
			// 从Session中获取验证码
			ActionContext ctx = ActionContext.getContext();
			Object obj = ctx
					.getSession()
					.get(SessionConstants.SESSION_VERCODE_KEYS.EMAIL_VERCODE_SEND_TIME);
			boolean sendMsg = false;
			if (obj == null) {
				// 倒计时为60秒
				this.countdown = COUNT_DOWN;
				// 将第一次获取验证码时间保存
				ctx.getSession()
						.put(SessionConstants.SESSION_VERCODE_KEYS.EMAIL_VERCODE_SEND_TIME,
								DateUtils.getCurTimeMillis());
				sendMsg = true;
			} else {
				// 第一次获取验证码时间
				...
				// 当前时间
				Long curDate = DateUtils.getCurTimeMillis();
				this.countdown = (curDate - emailVercodeSendTime) / 1000;
				this.countdown = COUNT_DOWN - this.countdown;
				// 超过60秒后，可重新发送,并且清空session
				if (this.countdown <= 0) {
					sendMsg = true;
			...
			if (sendMsg && email != null && !email.equals("")) {
				...
				ctx.getSession()
						.put(SessionConstants.SESSION_VERCODE_KEYS.EMAIL_SMS_MD5_VERCODE,
								MD5BASE64.base64MD5(email + emailSmsVercode,
										"UTF-8"));
				logger.info(email + ",验证码:" + emailSmsVercode);
				this.errorMsg = "";
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			logger.error("ForgotAction sendEmailVerCodeJson error : "
					+ ex.getMessage());
	...
	public String checkCountDownEmailJson() {
		try {
			...
			ex.printStackTrace();
			logger.error("ForgotAction checkCountDownEmailJson error : "
					+ ex.getMessage());
		}
	...
	public String toSetNewPwdByPhone() {
		try {
			...
			Object obj = ctx
					.getSession()
					.get(SessionConstants.SESSION_VERCODE_KEYS.PHONE_SMS_MD5_VERCODE);
			...
			this.phoneMd5Code = MD5BASE64.base64MD5(
					this.phone + this.phoneCode, "UTF-8");
			// 验证码不一致
			...
			logger.error("ForgotAction toSetNewPwdByPhone error : "
					+ ex.getMessage());
			ex.printStackTrace();
			return "checkError";
		}
	...
	public String toSetNewPwdByEmail() {
		try {
			...
			Object obj = ctx
					.getSession()
					.get(SessionConstants.SESSION_VERCODE_KEYS.EMAIL_SMS_MD5_VERCODE);
			...
			this.emailMd5Code = MD5BASE64.base64MD5(
					this.email + this.emailCode, "UTF-8");
			// 验证码不一致
			...
			logger.error("ForgotAction toSetNewPwdByEmail error : "
					+ ex.getMessage());
			ex.printStackTrace();
			return "checkError";
		}
	...
	public String modifyUserPwdByPhone() {
		try {
			if(!toSetNewPwdByPhone().equals("toSetNewPwdByPhone")){
				return "checkError";
			}
			this.type = "phone";
			// 从Session中获取验证码
			...
			if (this.ebccId == null || this.ebccId.equals("")) {
				...
				return "checkError";
			} else if (this.phonePassword == null
					|| this.phonePassword.equals("")) {
				setThisErrorMsg("密码不能为空！");
				return "checkError";
			} else if (this.phonePassword.length() > 16
					|| this.phonePassword.length() < 6) {
				// 密码长度为6到16位
			...
			if (this.phoneMd5Code.equals(phoneSmsMd5Vercode)) {
				...
				ctx.getSession().put(
						SessionConstants.SESSION_USER_INFO.USER_INFO, entity);
				// 设置登陆类型
				ctx.getSession().put(
						SessionConstants.SESSION_USER_INFO.LOGIN_TYPE,
						SessionConstants.SESSION_USER_INFO.PHONE);
				// 登录后，修改最后一次登录时间
				userService.modifyCustomerTimeById(entity);
				setThisErrorMsg("修改成功！");
			} else {
				this.errorType = "phone";
			...
			logger.error("ForgotAction modifyUserPwdByPhone error : "
					+ ex.getMessage());
			ex.printStackTrace();
			return "checkError";
		}
	...
	public String modifyUserPwdByEmail() {
		try {
			if(!toSetNewPwdByEmail().equals("toSetNewPwdByEmail")){
				return "checkError";
			}
			this.type = "email";
			// 从Session中获取验证码
			...
			if (this.ebccId == null || this.ebccId.equals("")) {
				...
				setThisErrorMsg("用户信息为空！");
				return "checkError";
			} else if (this.emailPassword == null
					|| this.emailPassword.equals("")) {
				setThisErrorMsg("密码不能为空！");
				return "checkError";
			} else if (this.emailPassword.length() > 16
					|| this.emailPassword.length() < 6) {
				// 密码长度为6到16位
				setThisErrorMsg("密码长度应在6-16位！");
				return "checkError";
			}
			...
			if (this.emailMd5Code.equals(phoneSmsMd5Vercode)) {
				...
				ctx.getSession().put(
						SessionConstants.SESSION_USER_INFO.LOGIN_TYPE,
						SessionConstants.SESSION_USER_INFO.EMAIL);
				// 登录后，修改最后一次登录时间
				userService.modifyCustomerTimeById(entity);
				setThisErrorMsg("修改成功！");
			} else {
			...
			logger.error("ForgotAction modifyUserPwdByEmail error : "
					+ ex.getMessage());
			ex.printStackTrace();
		}
		return "index";
	...
	public String getRegistVerCode() {
		return registVerCode;
	}
