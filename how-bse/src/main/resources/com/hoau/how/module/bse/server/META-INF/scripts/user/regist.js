...
function regist(){
	...
	if(accept.attr("checked") != "checked"){
		setErrorMsg("#accept_tips","请先阅读华宇网厅服务协议！");
		return;
	}
...
function checkPassword1(){
	...
	if(pw1!=""){ 
     	var pwd=/^(?=.{6,16}$)/;
     	if(!pwd.test(pw1)){
			setErrorMsg("#password_tips","密码必须为6-16个字符！");
            return false; 
        }else{
			$("#password_tips").html("<span class='icon_succ'></span>");
      		return true;
     	}
	}else{
		setErrorMsg("#password_tips","请输入密码！");
		return false; 	
	}
}
function checkPassword2(){
	...
	var pw2 = $("#password2").val();
 	if(pw2!=""){
     	var pwd=/^(?=.{6,16}$)/;
     	if(!pwd.test(pw2)){
			setErrorMsg("#password2_tips","密码必须为6-16个字符！");
            return false; 
        }else{
			if(pw1 == pw2) {
				$("#password2_tips").html("<span class='icon_succ'></span>");
				return true;
			}else {
				setErrorMsg("#password2_tips","确认密码和密码不一致！");
      			return false;
			}
     	} 
	}else{
		setErrorMsg("#password2_tips","请输入确认密码！");
		return false; 	
	}
}
function checkNetLogin(check){
	...
	if($ebccNetLogin.val()==$ebccNetLogin.attr('ov')){
		...
		if($ebccNetLogin.val().length > 50){
			setErrorMsg("#userName_tips","用户名长度不能超过50位！");
			return false;
		}else{
	...
	if(check){
	 	$.post("registAction!checkUserNameJson.action",{
			...
			function(data, status) {
				$("#userName_tips").html(data.errorMsg);
			}
		);
	}
	return true;
...
function checkVerCode(){
	...
	if($registVerCode.val() == $registVerCode.attr("ov")){
		setErrorMsg("#verCode_tips","请输入验证码！");
        return false; 
	}else{
		return true;
	}
	...
	$.post("registAction!sendPhoneVerCodeJson.action",{
		...
		function(data, status) {
			if(data.errorType == "phone"){
				...
				$("#verCode_tips").html(data.errorMsg);
				countDownPhoneNum = data.countdown;
				phoneTimer = setInterval(phoneCountdown, "1000");
			}
		}
