...
public class StringUtil {
	 ...
	 * @update
	 */
	public static List<String> textarea2List(String orgStr){
		if (EmptyUtils.isNotEmpty(orgStr)){
			//获取运单号数组
			orgStr= orgStr.replace("\r\n", ",");
			orgStr= orgStr.replace("<br/>", ",");

			String[] nos = orgStr.split(",");
			List<String> noList = new ArrayList<String>();
			for(int i = 0; i< (nos.length>10?10:nos.length); i++){
				if(null != nos[i] && !"".equals(nos[i].trim()))
				noList.add(nos[i].trim());
			}
			return noList;
		}else {
	...
	public static String timeMinusOne(String time){
		...
		if(time.indexOf("-") > 0){
			...
			Integer beginTime = Integer.parseInt(times[0]);
