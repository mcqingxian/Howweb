...
import com.hoau.wechat.util.UUIDGenerator;
/**
 * @author：莫涛
...
public class OrderService implements IOrderService {
	...
	@Resource
	private IBseOperationLogService iBseOperationLogService;
	@Override
	...
	public NetOrderEntity queryOneOrder(Integer cbccId) {
		NetOrderEntity entity = netOrderDao.queryOrderDetail(cbccId);
	...
	public void createOrder(NetOrderEntity entity){
		...
		if(entity.getEinoId() == null || entity.getEinoId().equals("")){
			...
			this.netOrderDao.saveOrder(entity);
			iBseOperationLogService.insert(entity,2);
		}else{
			this.netOrderDao.updateOrder(entity);
		}
	}
	...
	public void copyNetOrderToEiNetOrder(NetOrderEntity entity, EiNetOrderReqModel reqModel,ObjectFactory objectFactory,int batchOrder) {
		...
		//订单来源
		reqModel.setEinoOrign(objectFactory.createEiNetOrderReqModelEinoOrign("NET_ORDER"));
		//异常类型
