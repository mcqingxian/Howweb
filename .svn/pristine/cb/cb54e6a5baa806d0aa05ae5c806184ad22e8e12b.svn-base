	...
	public String sendPhoneVerCodeJson(){
		try{
			...
			if(obj == null){
				...
				//将第一次获取验证码时间保存
				ctx.getSession().put(SessionConstants.SESSION_REGIST_VERCODE_KEYS.PHONE_REGIST_VERCODE_SEND_TIME,DateUtils.getCurYmdhms());
				sendMsg = true;
			}else{
				//第一次获取验证码时间
			...
			if(sendMsg && this.entity != null && this.entity.getEbccMobile() != null && !this.entity.getEbccMobile().equals("")){
				...
				if(sessionCode == null){
					sessionCode = this.registVerCode;
				}
