...
import com.hoau.wechat.constants.ControllConstants;
...
public class PersonalDataAction extends AbstractAction{
	...
	private Logger logger = Logger.getLogger(PersonalDataAction.class);
	@Resource
	...
	public String modifyPhone(){
		...
		try {
			...
			if(StringUtil.isEmpty(entity.getEbccMobile())){
				setThisErrorMsg("手机号不能为空！");
				return index;
			}
			//如果当前手机号与数据库中记录不一致
			if(!userInfo.getEbccMobile().equals(this.entity.getEbccMobile())){
				...
				Object obj1 = ctx.getSession().get(SessionConstants.SESSION_REGIST_VERCODE_KEYS.PHONE_REGIST_PHONE);
				if(obj == null || obj1 == null){
					setThisErrorMsg("验证码信息为空，请先获取验证码！");
					return index;
				}
				if(!obj1.equals(entity.getEbccMobile())){
					setThisErrorMsg("请输入正确的手机号！");
					return index;
				}
				...
				// 修改手机号
				userInfo.setEbccMobile(entity.getEbccMobile());
				this.userService.modifyCustomerInfoById(userInfo);
				ctx.getSession().put(SessionConstants.SESSION_USER_INFO.USER_INFO, userInfo);
			}
		} catch (Exception e) {
			logger.error("PersonalDataAction modifyPhone error : " + e.getMessage());
			this.errorType = "systemError";
			setThisErrorMsg("系统异常："+e.getMessage());
			e.printStackTrace();
			return index;
		}
		return index;
